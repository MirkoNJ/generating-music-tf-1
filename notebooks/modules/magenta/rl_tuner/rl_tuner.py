...
from modules.magenta.rl_tuner import rl_tuner_eval_metrics
from modules.magenta.rl_tuner import rl_tuner_ops
from modules.magenta.music import melodies_lib as mlib
...
class RLTuner(object):
  ...
  def __init__(self, output_dir, music_dir,
    ...
    if initialize_immediately:
      self.initialize_internal_models_graph_session(restore_from_checkpoint=True) #False for RL only
    ...
    with self.graph.as_default():
      ...
      if restore_from_checkpoint:
        ...
        if np.sum((q1 - reward1)**2) == 0.0:
          tf.logging.info('Successfully initialized internal nets from checkpoint!')
        else:
  ...
  def prime_internal_model(self, model):
    ...
    """
    model.state_value_t = model.get_zero_state_t()
    model.state_value_n = model.get_zero_state_n()


    if self.priming_mode == 'random_midi':
      #TODO(Mirko): Not working
      priming_idx = np.random.randint(0, len(self.priming_states))
      model.state_value_t = np.reshape(
          self.priming_states[priming_idx, :],
          (1, model.cell_t.state_size))
  ...
  def build_graph(self):
    ...
    with tf.name_scope('reward_computation'):
      ...
      # self.reward_scores = tf.Print(self.reward_scores, ["Reward scores: ", tf.reduce_min(self.reward_scores)])
      # self.reward_scores = tf.Print(self.reward_scores, ["Reward scores: ", tf.reduce_max(self.reward_scores)])
    ...
    with tf.name_scope('taking_action'):
      ...
      # self.action_scores = tf.Print(self.action_scores, ["Action scores: ", tf.round(self.action_scores)])
    ...
    with tf.name_scope('estimating_future_rewards'):
      ...
      # self.next_action_scores = tf.Print(self.next_action_scores, ["Next action scores: ", tf.round(self.next_action_scores)])
      tf.summary.histogram(
    ...
    with tf.name_scope('q_value_prediction'):
      ...
      # receive.
      # self.masked_action_scores = tf.Print(self.masked_action_scores, ["tmp diff: ",temp_diff])
      self.prediction_error = tf.reduce_mean(tf.square(temp_diff))

      # self.prediction_error = tf.Print(self.prediction_error, ["Number of times trained: ", tf.constant(self.num_times_train_called)])
      # self.prediction_error = tf.Print(self.prediction_error, ["Predicion error shape: ",self.prediction_error])
      # self.prediction_error = tf.Print(self.prediction_error, ["Masked action scores: ", tf.round(self.masked_action_scores)], summarize=40)
      # self.prediction_error = tf.Print(self.prediction_error, ["Next action scores: ", tf.round(self.next_action_scores)], summarize=40)
      # self.prediction_error = tf.Print(self.prediction_error, ["Target values: ", tf.round(self.target_vals)], summarize=40)
      # self.prediction_error = tf.Print(self.prediction_error, ["Rewards: ",tf.round(self.rewards)], summarize=40)

      # Compute gradients
      self.params = tf.trainable_variables()
      self.prediction_error = tf.Print(self.prediction_error, ["Predicion error: ", tf.round(self.prediction_error)])
      self.gradients = self.optimizer.compute_gradients(self.prediction_error)

      # Clip gradients.
      for i, (grad, var) in enumerate(self.gradients):
        if grad is not None:
          epsilon = tf.constant(1e-14)
          ...
          self.gradients[i] = (tf.clip_by_value(grad, -grad*5 / tf.sqrt(tf.reduce_sum(tf.square(grad)) + epsilon), grad*5 / tf.sqrt(tf.reduce_sum(tf.square(grad)) + epsilon)), var)
          # if i == 0:
          #   self.prediction_error = tf.Print(self.prediction_error, ["Gradients: ", self.gradients[i][0]])


      for grad, var in self.gradients:
        tf.summary.histogram(rl_tuner_ops.trim_variable_postfixes(var.name), var)
        if grad is not None:
          tf.summary.histogram(rl_tuner_ops.trim_variable_postfixes(var.name) + '/gradients', grad)
  ...
  def train(self, num_steps=10000, exploration_period=5000, enable_random=True):
    ...
    tf.logging.info('Evaluating model at %s...', str(current_time_str))
    self.evaluate_model()
    current_time_str = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    tf.logging.info('Finished model evaluation at %s!', str(current_time_str))
    ...
    for i in range(num_steps):
      ...
      state_t = np.array(self.q_network.state_value_t).flatten()
      state_n = np.array(self.q_network.state_value_n).flatten()
      # state_event = np.array(self.q_network.state_value_event).flatten()
      # self.q_network.state_value_t: tuple(map(tuple, x.reshape(2,2,38,200))) self.hparams.one_hot_length, self.rnn_layer_sizes_n[2]

      action, new_observation, reward_scores = self.action(
          last_observation, exploration_period, enable_random=enable_random,
          sample_next_obs=sample_next_obs)

      new_state_t = np.array(self.q_network.state_value_t).flatten()
      new_state_n = np.array(self.q_network.state_value_n).flatten()
      # new_state_event = np.array(self.q_network.state_value_event).flatten()
      new_reward_state_t = np.array(self.reward_rnn.state_value_t).flatten()
      new_reward_state_n = np.array(self.reward_rnn.state_value_n).flatten()
      # new_reward_state_event = np.array(self.reward_rnn.state_value_event).flatten()
      ...
      self.store(last_observation, state_t, state_n, action, reward, new_observation, # , state_event
                 new_state_t, new_state_n, new_reward_state_t, new_reward_state_n) # , new_state_event, , new_reward_state_event
      ...
      if i > 0 and i % self.output_every_nth == 0:
        ...
        tf.logging.info('\tReward for last %s steps: %s',
                        self.output_every_nth, self.reward_last_n)
        ...
        if self.exploration_mode == 'egreedy':
          exploration_p = rl_tuner_ops.linear_annealing(
              i , exploration_period, 1.0,
              self.dqn_hparams.random_action_probability) # self.actions_executed_so_far
  ...
  def action(self, observation, exploration_period=0, enable_random=True,
    ...
    (action, action_softmax, self.q_network.state_value_t, self.q_network.state_value_n, # self.q_network.state_value_event,
     reward_scores, self.reward_rnn.state_value_t, self.reward_rnn.state_value_n) = self.session.run( # , self.reward_rnn.state_value_event
         [self.predicted_actions, self.action_softmax,
          self.q_network.state_tensor_t, self.q_network.state_tensor_n, self.reward_scores, # self.q_network.state_tensor_event,
          self.reward_rnn.state_tensor_t, self.reward_rnn.state_tensor_n], # , self.reward_rnn.state_tensor_event
         {self.q_network.Note_State_Batch: self.q_network.get_input_batch(observation, self.beat),
          self.q_network.timewise_state: self.q_network.state_value_t,
          self.q_network.notewise_state: self.q_network.state_value_n,
          # self.q_network.event_state: self.q_network.state_value_event,
          self.q_network.output_keep_prob: 0.5,
          self.reward_rnn.Note_State_Batch: self.reward_rnn.get_input_batch(observation, self.beat),
          self.reward_rnn.timewise_state: self.reward_rnn.state_value_t,
          self.reward_rnn.notewise_state: self.reward_rnn.state_value_n,
          # self.reward_rnn.event_state: self.reward_rnn.state_value_event,
          self.reward_rnn.output_keep_prob: 0.5
          })
  ...
  def store(self, observation, state_t, state_n, action, reward, newobservation, newstate_t, newstate_n, # , state_event , newstate_event
            new_reward_state_t, new_reward_state_n): # , new_reward_state_event
    ...
    if self.num_times_store_called % self.dqn_hparams.store_every_nth == 0:
      self.experience.append((observation, state_t, state_n, action, reward, # , state_event
                              newobservation, newstate_t, newstate_n, new_reward_state_t, new_reward_state_n)) # , newstate_event, , new_reward_state_event
  ...
  def training_step(self):
    ...
    target_q_network.
    """
    #Think about whether it makes sense to reset the Note_State_Batch_value
    if self.num_times_train_called % self.dqn_hparams.train_every_nth == 0:
      ...
      # Batch states.(self.hparams.midi_high + 1 - self.hparams.midi_low)
      states_t = np.empty((len(samples), 2*2*(self.q_network.hparams.midi_high + 1- self.q_network.hparams.midi_low)*self.q_network.hparams.rnn_layer_sizes_t[0]))
      states_n = np.empty((len(samples), 2*2*self.q_network.hparams.rnn_layer_sizes_n[0]))
      # states_event = np.empty((len(samples), 2*(self.q_network.hparams.midi_high + 1- self.q_network.hparams.midi_low)*100))
      new_states_t = np.empty((len(samples), 2*2*(self.target_q_network.hparams.midi_high + 1- self.target_q_network.hparams.midi_low)*self.target_q_network.hparams.rnn_layer_sizes_t[0]))
      new_states_n = np.empty((len(samples), 2*2*self.target_q_network.hparams.rnn_layer_sizes_n[0]))
      # new_states_event = np.empty((len(samples), 2*(self.target_q_network.hparams.midi_high + 1- self.target_q_network.hparams.midi_low)*100))
      reward_new_states_t = np.empty((len(samples), 2*2*(self.reward_rnn.hparams.midi_high + 1- self.reward_rnn.hparams.midi_low)*self.reward_rnn.hparams.rnn_layer_sizes_t[0]))
      reward_new_states_n = np.empty((len(samples), 2*2*self.reward_rnn.hparams.rnn_layer_sizes_n[0]))
      # reward_new_states_event = np.empty((len(samples), 2*(self.reward_rnn.hparams.midi_high + 1- self.reward_rnn.hparams.midi_low)*100))
      input_batch = np.empty((len(samples), (self.q_network.hparams.midi_high + 1 - self.q_network.hparams.midi_low), 1, 4))
      input_batch_new = np.empty((len(samples), (self.target_q_network.hparams.midi_high + 1 - self.target_q_network.hparams.midi_low), 1, 4))
      action_mask = np.zeros((len(samples), self.num_actions))
      rewards = np.empty((len(samples),))

      for i, (o, s_t, s_n, a, r, new_o, new_s_t, new_s_n, reward_s_t, reward_s_n) in enumerate(samples): # s_e, new_s_e, reward_s_e
        input_batch[i, :,:,:] = self.q_network.get_input_batch(o, self.beat)
        input_batch_new[i, :,:,:] = self.target_q_network.get_input_batch(new_o, self.beat)
        states_t[i, :] = s_t
        states_n[i, :] = s_n
        # states_event[i, :] = s_e
        new_states_t[i, :] = new_s_t
        new_states_n[i, :] = new_s_n
        # new_states_event[i, :] = new_s_e
        action_mask[i, :] = a
        rewards[i] = r
        reward_new_states_t[i, :] = reward_s_t
        reward_new_states_n[i, :] = reward_s_n
        # reward_new_states_event[i, :] = reward_s_e

      states_t = tuple(map(tuple, states_t.reshape(2, 2, (self.q_network.hparams.midi_high + 1- self.q_network.hparams.midi_low)*len(samples), self.q_network.hparams.rnn_layer_sizes_t[0])))
      states_n = tuple(map(tuple, states_n.reshape(2, 2, len(samples), self.q_network.hparams.rnn_layer_sizes_n[0])))
      # states_event = tuple(map(tuple, states_event.reshape(1, 2, (self.q_network.hparams.midi_high + 1- self.q_network.hparams.midi_low)*len(samples), 100)))
      new_states_t = tuple(map(tuple, new_states_t.reshape(2, 2, (self.target_q_network.hparams.midi_high + 1- self.target_q_network.hparams.midi_low)*len(samples), self.target_q_network.hparams.rnn_layer_sizes_t[0])))
      new_states_n = tuple(map(tuple, new_states_n.reshape(2, 2, len(samples), self.target_q_network.hparams.rnn_layer_sizes_n[0])))
      # new_states_event = tuple(map(tuple, new_states_event.reshape(1, 2, (self.target_q_network.hparams.midi_high + 1- self.target_q_network.hparams.midi_low)*len(samples), 100 )))
      reward_new_states_t = tuple(map(tuple, reward_new_states_t.reshape(2, 2, (self.reward_rnn.hparams.midi_high + 1- self.reward_rnn.hparams.midi_low)*len(samples), self.reward_rnn.hparams.rnn_layer_sizes_t[0])))
      reward_new_states_n = tuple(map(tuple, reward_new_states_n.reshape(2, 2, len(samples), self.reward_rnn.hparams.rnn_layer_sizes_n[0])))
      # reward_new_states_event = tuple(map(tuple, reward_new_states_event.reshape(1, 2, (self.reward_rnn.hparams.midi_high + 1- self.reward_rnn.hparams.midi_low)*len(samples), 100)))
      ...
      _, _, target_vals, summary_str = self.session.run([
          ...
          self.q_network.Note_State_Batch: input_batch,
          self.q_network.timewise_state: states_t,
          self.q_network.notewise_state: states_n,
          # self.q_network.event_state: states_event,
          self.q_network.output_keep_prob: 0.5,
          self.target_q_network.Note_State_Batch: input_batch_new,
          self.target_q_network.timewise_state: new_states_t,
          self.target_q_network.notewise_state: new_states_n,
          # self.target_q_network.event_state: new_states_event,
          self.target_q_network.output_keep_prob: 0.5,
  ...
  def collect_reward(self, obs, action, reward_scores):
    ...
    # print("Note Rnn reward:")
    # print(note_rnn_reward)
  ...
  def reward_from_reward_rnn_scores(self, action, reward_scores, epsilon =  1e-14):
    ...
    normalization_constant = scipy.special.logsumexp(reward_scores+ epsilon)
  ...
  def generate_music_sequence(self, title='rltuner_sample',
    ...
    for i in range(length):
      softmax, self.q_network.state_value_t, self.q_network.state_value_n = self.session.run( # ,  self.q_network.state_value_event
          [self.action_softmax, self.q_network.state_tensor_t, self.q_network.state_tensor_n], # , self.q_network.state_tensor_event
          {self.q_network.Note_State_Batch: self.q_network.get_input_batch(next_obs,i),
            self.q_network.timewise_state: self.q_network.state_value_t,
            self.q_network.notewise_state: self.q_network.get_zero_state_n(),
            # self.q_network.event_state: self.q_network.state_value_event,
            self.q_network.output_keep_prob: 0.5})
    ...
    filename = rl_tuner_ops.get_next_file_name(self.music_dir, title, 'mid')
    midi_io.sequence_proto_to_midi_file(sequence, filename)

    tf.logging.info('Wrote a melody to %s', self.music_dir)
