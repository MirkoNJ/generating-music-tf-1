...
import modules.magenta.common as common
import modules.magenta.music as music
...
def make_rnn_cell(rnn_layer_sizes,
  ...
  for i in range(len(rnn_layer_sizes)):
    cell = base_cell(rnn_layer_sizes[i], state_is_tuple=state_is_tuple)
    ...
    if residual_connections:
      ...
      if i == 0 or rnn_layer_sizes[i] != rnn_layer_sizes[i - 1]:
        cell = tf.contrib.rnn.InputProjectionWrapper(cell, rnn_layer_sizes[i], state_is_tuple=state_is_tuple)
...
def get_build_graph_fn(mode, config, sequence_example_file_paths=None):
  ...
  def build():
    ...
    if mode in ('train', 'eval'):
      ...
      inputs, labels, lengths = common.get_padded_batch(
    ...
    if isinstance(encoder_decoder,
                  music.OneHotIndexEventSequenceEncoderDecoder):
    ...
    outputs_flat = common.flatten_maybe_padded_sequences(
        outputs, lengths)
    ...
    if mode in ('train', 'eval'):
      labels_flat = common.flatten_maybe_padded_sequences(
          labels, lengths)
